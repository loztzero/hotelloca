...
<div class="container" ng-controller="MainCtrl">
	...
	<a href="{{ url('admin/hotel-vs-user/room?hotel=') . $profile->id }}" class="button">Back</a>

	<div class="travelo-box">
		<h3>Hotel Rooms</h3>
		@include('layouts.message-helper')

		<form class="form-horizontal" role="form" method="POST" action="{{ url('admin/hotel-vs-user/save-room') }}" enctype="multipart/form-data">
			<input type="hidden" name="_token" value="{{ csrf_token() }}">
			<input type="hidden" name="hotel" value="{{ Request::get('hotel') }}">
		    		...
		    		<div class="form-group">
						<div class="col-xs-12">
				          	<a class="th" role="button" aria-label="Thumbnail" href="{{ url().'/uploads/hotels/'. old('mst020_id'). '/'.old('id').'/room.jpg' }}" data-lightbox="image-1" data-title="{{ old('room_name') }}">
								<img src="{{ url().'/uploads/hotels/'. old('mst020_id'). '/'.old('id').'/room.jpg' }}" height="150">
							</a>
				         </div>
				    </div>
			    @endif
